 ...
 * startup.
 *
 * A service can only be registered a single time; registering a service more
 * than once will cause an error to be thrown.
 * @param {function} T Closure or class that implements {@link AbstractService}
 * @example
...
{
  if (~this._services.indexOf(T))
    throw new Error('cannot register service ' + getName(T) +
      ': already registered');
 ...
 * @param {any} thing A class constructor or object instance
 * @return {IoCBinding}
 */
 ...
 * @param {string} tag Tag to register this dependency under
 * @param {object} thing The object instance to register
 * @return {IoCBinding}
 */
 ...
 * @param {Function|string} thing The thing to build via this app harness
 * @return {object}
 * @example
 * // create something via its string tag
 * var db = app.make('db');
 * @example
 * // call a closure with dependencies resolved and injected
 * var result = app.make(function(users, currentUserId) {
 *   return users.fetch(currentUserId);
 * });
 * @example
 * // Build an object via a class constructor with constructor parameter
 * // injection
 * var instance = app.make(MyClass);
 */
